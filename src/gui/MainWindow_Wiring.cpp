...
void MainWindow::wireVfoWidget(VfoWidget* w, SliceModel* s)
{
    ...
    // close the dialog instead — its destroyed hook (ensureAetherRxDialog)
    ...
    // ensureAetherRxDialog() directly.
    connect(w, &VfoWidget::aetherDspRequested, this, [this, w] {
        if (m_rxDialog && m_rxDialog->isVisible() && m_dspPopoutVfo == w) {
            m_rxDialog->close();  // destroyed hook re-opens the inline panel
            return;
        }
    ...
    // panel back inline (destroyed hook in ensureAetherRxDialog()).
